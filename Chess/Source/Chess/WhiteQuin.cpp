...
bool AWhiteQuin::LineTrace(FVector Direction)
{	
			...
			{	
				if (LineTrace(BlackKing->GetActorLocation()) or m_CanAttackKing)
				{
					if (LineTrace(BoardPiece->GetActorLocation()))
					{
						...
						FVector KingDifference = BlackKing->GetActorLocation() - BoardPiece->GetActorLocation();
						FVector QuenDifference = BoardPiece->GetActorLocation() - CurrentLocation;


						FVector QueenLocation = GetActorLocation();
						FVector KingLocation = BlackKing->GetActorLocation();
						FVector BoardPieceLocation = BoardPiece->GetActorLocation();

						FVector KingQueenDifference = KingLocation - QueenLocation;

						bool IsInInterval = false;

						if (FMath::Abs(QueenLocation.Y - KingLocation.Y) < 20)
						{	
							IsInInterval = ((BoardPiece->GetActorLocation().X >= BlackKing->GetActorLocation().X and BoardPiece->GetActorLocation().X < CurrentLocation.X
								or BoardPiece->GetActorLocation().X <BlackKing->GetActorLocation().X and BoardPiece->GetActorLocation().X > CurrentLocation.X)
								and (BoardPiece->GetActorLocation().Y > BlackKing->GetActorLocation().Y - 100 and BoardPiece->GetActorLocation().Y < CurrentLocation.Y + 100
									or BoardPiece->GetActorLocation().Y < BlackKing->GetActorLocation().Y + 100 and BoardPiece->GetActorLocation().Y > CurrentLocation.Y + 100));
						}
						else if (FMath::Abs(QueenLocation.X - KingLocation.X) < 20)
						{	
							IsInInterval = ((BoardPiece->GetActorLocation().X > BlackKing->GetActorLocation().X - 100 and BoardPiece->GetActorLocation().X < CurrentLocation.X + 100
								or BoardPiece->GetActorLocation().X < BlackKing->GetActorLocation().X + 100 and BoardPiece->GetActorLocation().X > CurrentLocation.X - 100)
						...
						else if((FMath::Abs(KingQueenDifference.X) >= FMath::Abs(KingQueenDifference.Y) - 500)
							&& (FMath::Abs(KingQueenDifference.X) <= FMath::Abs(KingQueenDifference.Y) + 500))
						{	
							IsInInterval = ((FMath::Abs(KingDifference.X) >= FMath::Abs(KingDifference.Y) - 100)
								&& (FMath::Abs(KingDifference.X) <= FMath::Abs(KingDifference.Y) + 100)
								and (FMath::Abs(QuenDifference.X) >= FMath::Abs(QuenDifference.Y) - 100)
								&& (FMath::Abs(QuenDifference.X) <= FMath::Abs(QuenDifference.Y) + 100))
								and ((BoardPiece->GetActorLocation().X > BlackKing->GetActorLocation().X - 100 and BoardPiece->GetActorLocation().X < CurrentLocation.X + 100
									or BoardPiece->GetActorLocation().X < BlackKing->GetActorLocation().X + 100 and BoardPiece->GetActorLocation().X > CurrentLocation.X - 100)
									and (BoardPiece->GetActorLocation().Y > BlackKing->GetActorLocation().Y and BoardPiece->GetActorLocation().Y < CurrentLocation.Y
										or BoardPiece->GetActorLocation().Y < BlackKing->GetActorLocation().Y and BoardPiece->GetActorLocation().Y > CurrentLocation.Y));
						}
						...
						if (IsInInterval and bIsValidMove)
						{
...
void AWhiteQuin::MoveUnderAttack()
{
	if (!IsDead)
	{
...
void AWhiteQuin::TurnOffLocations()
{
	TArray<AActor*> FoundActors;
	UGameplayStatics::GetAllActorsOfClass(GetWorld(), AOnePeaceOfBoard::StaticClass(), FoundActors);
...
void AWhiteQuin::move(FVector Direction)
{
	if (LineTrace(FVector(Direction.X, Direction.Y, GetActorLocation().Z)))
	{	
		bool bIsValidMove = false;
...
bool AWhiteQuin::CanAttack(FVector Direction)
{	
...
void AWhiteQuin::Attack(ABlackChessFigure* BlackFigure)
{	
...
bool AWhiteQuin::CanMove(FVector Direction)
{	
