...
bool ABlackQuin::LineTrace(FVector Direction)
{	
			...
			{	
				if (LineTrace(WhiteKing->GetActorLocation()) or m_CanAttackKing)
				{
					if (LineTrace(BoardPiece->GetActorLocation()))
					{
						...
						FVector Difference = WhiteKing->GetActorLocation() - CurrentLocation;
						FVector KingDifference = WhiteKing->GetActorLocation() - BoardPiece->GetActorLocation();
...
void ABlackQuin::MoveUnderAttack()
{	
	if (!IsDead)
	{
...
void ABlackQuin::TurnOffLocations()
{
	TArray<AActor*> FoundActors;
	UGameplayStatics::GetAllActorsOfClass(GetWorld(), AOnePeaceOfBoard::StaticClass(), FoundActors);
...
void ABlackQuin::move(FVector Direction)
{
	if (LineTrace(FVector(Direction.X, Direction.Y, GetActorLocation().Z)))
	{
		bool bIsValidMove = false;
...
bool ABlackQuin::CanAttack(FVector Direction)
{	
...
void ABlackQuin::Attack(AWhiteChessFigure* WhiteFigure)
{
...
bool ABlackQuin::CanMove(FVector Direction)
{	
